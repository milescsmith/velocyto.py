...
# NOTE: neither of this functions are used
...
#         Either the original segment match or a fake Segment matches
#         that will take the place of a set segment that are spliced

#     Note
#     ----
#     This does not take in consideration all the corner cases it is very
#     difficoult without keeping track of the splicing event
#     """
        ...
        if sm.is_spliced:
            ...
            while segments_list[0].is_spliced:
                ...
                if sum(s.segment[1] - s.segment[0] + 1 for s in sm_list) + segments_list[0] > read_len:
                    break
            if len(segments_list) != 2:  # noqa PLR2004
                # just for safety let's ignore those counts otherwise we could make a mess
                continue
