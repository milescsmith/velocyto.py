...
EXON_SEGMENT_VALUE = 101
INTRON_SEGMENT_VALUE = 105

MIN_UCELL_SIZE = 2

MIN_PCA_VARIANCE_RATIO = 0.002

MIN_TMP_COLORANDUM = 0.00005
MAX_TMP_COLORANDUM = 10

MIN_CELL_EVENT_COUNTS = 80

LONGEST_EXTANT_LOCUS = 3000000  # longest known locus
...
# Why hardcode, when we could just read in the json file of barcodes?
# Chromium_v1_barcodes_file = ir.files("barcodes").joinpath("chromium-single-cell-sample-indexes-plate-v1.json.gz")
# Chromium_v2_barcodes_file = ir.files("barcodes").joinpath("chromium-shared-sample-indexes-plate.json.gz")
# GEMcode_barcodes_file = ir.files("barcodes").joinpath("gemcode-single-cell-sample-indexes-plate.json.gz")

# with ir.as_file(Chromium_v1_barcodes_file) as v1, ir.as_file(Chromium_v2_barcodes_file) as v2, ir.as_file(GEMcode_barcodes_file) as gem:
#     # I can't seem to get `object_hook` or a custom JSONDecoder class to work and the files from 10X are JSON arrays
#     # so, we do this
#     v1_indices = {k:v for k, v in json.load(gzip.open(v1))}
#     v2_indices = {k:v for k, v in json.load(gzip.open(v2))}
#     gemcode_indices = {k:v for k, v in json.load(gzip.open(gem))}
