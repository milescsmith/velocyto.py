...
class VelocytoLoom:
    ...
    def plot_fractions(self, save2file: str = None) -> None:
        ...
        for i in np.unique(chip_ix):
            ...
            plt.ylabel("Fraction")
            plt.bar(
                np.linspace(-0.2, 0.2, n)[i] + np.arange(3),
                _mean,
                0.5 / (n * 1.05),
                label=chips[i],
            )
    ...
    def score_detection_levels(
        self,
        min_expr_counts: int = 50,
    ...
    def filter_genes(
        self,
        by_detection_levels: bool = False,
        by_cluster_expression: bool = False,
        by_cv_vs_mean: bool = False,
        by_custom_array: Any = None,
    ...
    def normalize_by_total(
        self,
        min_perc_U: float = 0.5,
        plot: bool = False,
        skip_low_U_pop: bool = True,
        same_size_UnS: bool = False,
    ) -> None:
        ...
        if skip_low_U_pop:
            self._normalize_U(
                relative_size=np.clip(self.initial_Ucell_size, min_Ucell_size, None),
    ...
    def normalize_median(self, which: str = "imputed", skip_low_U_pop: bool = True) -> None:
        ...
        if which == "renormalize":
            ...
            if skip_low_U_pop:
                self.U_sz[:, ~self.small_U_pop] = self.U_sz[:, ~self.small_U_pop] * (
                    np.median(self.U_sz[:, ~self.small_U_pop].sum(0)) / self.U_sz[:, ~self.small_U_pop].sum(0)
                )
            else:
            ...
            if skip_low_U_pop:
                ...
                self.Ux_sz[:, ~self.small_U_pop] = self.Ux[:, ~self.small_U_pop] * (
                    np.median(self.Ux[:, ~self.small_U_pop].sum(0)) / self.Ux[:, ~self.small_U_pop].sum(0)
                )
            else:
    ...
    def gene_knn_imputation(
        self,
        k: int = 15,
        pca_space: float = False,
    ...
    def fit_gammas(
        ...
        if fit_offset:
            ...
            if weighted:
                self.gammas, self.q = fit_slope_weighted_offset(
                    tmpU[:, self.steady_state],
                    tmpS[:, self.steady_state],
                    W,
                    fixperc_q=True,
            ...
            if weighted:
                self.gammas, self.R2 = fit_slope_weighted(
                    tmpU[:, self.steady_state],
                    tmpS[:, self.steady_state],
                    W,
                    return_R2=True,
    ...
    def estimate_transition_prob(
        ...
        **kwargs,
        ...
        if knn_random:
            ...
            self.embedding_knn = sparse.csr_matrix(
                (
                    np.ones(nonzero),
                    neigh_ixs.ravel(),
            ...
            if transform == "linear":
                ...
                if calculate_randomized:
                    ...
                    self.corrcoef_random = colDeltaCorLog10partial(
                        hi_dim,
                        np.log10(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm),
                        neigh_ixs,
                        threads=threads,
                        psc=psc,
                    )
                ...
                if calculate_randomized:
                    ...
                    self.corrcoef_random = colDeltaCorSqrtpartial(
                        hi_dim,
                        np.sqrt(np.abs(delta_hi_dim_rndm) + psc) * np.sign(delta_hi_dim_rndm),
                        neigh_ixs,
                        threads=threads,
                        psc=psc,
                    )
    ...
    def calculate_embedding_shift(
        ...
        expression_scaling: bool = True,
        scaling_penalty: float = 1.0,
    ) -> None:
    ...
    def calculate_grid_arrows(
        ...
        UZ = (delta_embedding[neighs] * gaussian_w[:, :, None]).sum(1) / np.maximum(1, self.total_p_mass)[
            :, None
        ]  # weighed average
        ...
        **quiver_kwargs: dict
        ...
        if plot_random:
            if min_magnitude is None:
                ...
                if not plot_dots:
                    UV_rndm = UV_rndm[
                        ~(mass_filter | (self.flow_norm_magnitude_rndm < min_magnitude)),
                        :,
                    ]
                    XY = XY[
                        ~(mass_filter | (self.flow_norm_magnitude_rndm < min_magnitude)),
        ...
        **quiver_kwargs: dict
        ...
        if plot_random and hasattr(self, "delta_embedding_random"):
            ...
            plt.quiver(
                ...
                self.delta_embedding_random[ix_choice, 0],
                self.delta_embedding_random[ix_choice, 1],
                scale=quiver_scale,
                **_quiver_kwargs,
            )
            plt.axis("off")
        ...
        plt.quiver(
            self.embedding[ix_choice, 0],
            self.embedding[ix_choice, 1],
            self.delta_embedding[ix_choice, 0],
            self.delta_embedding[ix_choice, 1],
            scale=quiver_scale,
            **_quiver_kwargs,
        )
        plt.axis("off")

    def plot_cell_transitions(
        self,
        cell_ix: int = 0,
        alpha: float = 0.1,
        ...
        plt.scatter(self.embedding[:, 0], self.embedding[:, 1], c=colorandum, s=50, edgecolor="", cmap=cmap)
        if mark_cell:
            plt.scatter(
                self.embedding[cell_ix, 0],
                self.embedding[cell_ix, 1],
                facecolor="none",
                s=100,
                edgecolor="k",
            )
        if plot_arrow:
            plt.arrow(
                self.embedding[cell_ix, 0],
                self.embedding[cell_ix, 1],
                self.delta_embedding[cell_ix, 0],
                self.delta_embedding[cell_ix, 1],
    ...
    def plot_velocity_as_color(
        ...
        **kwargs: dict
        ...
        scatter_viz(tsne[:, 0], tsne[:, 1], c=cmap(tmp_colorandum), **kwarg_plot)
        plt.axis("off")
...
def scale_to_match_median(sparse_matrix: sparse.csr_matrix, genes_total: np.ndarray) -> sparse.csr_matrix:
    ...
    return sparse.csc_matrix(
        (newdata, sparse_matrix.indices, sparse_matrix.indptr),
